...
export class AuthService {
    constructor(
        ...
        private sendMailService: SendMailService, // private jwtService: JwtService
    ) { }
        ...
        try {
            ...
            if (error instanceof HttpException) {
                ...
                throw new InternalServerErrorException(
                    'Error while creating new user account',
                );
            }
        }
    }
    ...
    async sendMail(
        ...
        try {
            const isMailSend = await this.sendMailService.sendMail(
                subject,
                html,
                email,
            );
    ...
    async expiredForgotToken(id: string, time: number) {
        try {
            ...
            throw new InternalServerErrorException(
                'Error while expiring forgot token',
            );
        }
    }
    ...
    async resetPassword(token, resetPasswordDto) {
        ...
        const match = await this.tokensModel.findOne({
            token: token,
    ...
    async updatePassword(req, updatepassword) {
        ...
        try {
            if (user) {
                ...
                if (password == confirmPassword) {
                    const hashPassword = await bcrypt.hash(updatepassword.password, 10);
                    ...
                    return { message: "your password and confirmPassword don't match" }
